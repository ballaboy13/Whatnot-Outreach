(function () {
   ...
   function saveData() {
          ...
          function hideStatus() { document.getElementById("status").className = "status hidden"; }
          function initMap() {
                      ...
                      d3.json("https://cdn.jsdelivr.net/npm/us-atlas@3/states-10m.json").then(function (us) {
                                    ...
                                    mapReady = true;
                                    loadData(); updateShowFilter(); renderPins(); updateStats();
                      });
          ...
          function updateShowFilter() {
                      ...
                      for (var i = 0; i < allData.shows.length; i++) {
                                    var opt = document.createElement("option"); opt.value = allData.shows[i]; opt.textContent = allData.shows[i]; select.appendChild(opt);
                      }
          ...
          function renderPins() {
                      ...
                      for (var j = 0; j < entries.length; j++) {
                                    ...
                                    else { var r = Math.min(6 + Math.log2(data.count) * 4, 22); pinG.append("circle").attr("r", r).attr("class", "pin-cluster"); pinG.append("text").attr("class", "pin-label").text(data.count); }
                                    (function(d) {
                                                    pinG.on("mouseover", function (event) { tooltip.innerHTML = "<h4>" + d.city + ", " + d.state + "</h4><p>" + d.count + " shipment" + (d.count > 1 ? "s" : "") + "</p><p>Shows: " + d.shows.join(", ") + "</p>"; tooltip.className = "tooltip"; tooltip.style.left = event.offsetX + 12 + "px"; tooltip.style.top = event.offsetY - 10 + "px"; });
          ...
          function updateStats() {
                      ...
                      for (var i = 0; i < shipments.length; i++) {
                                    ...
                                    cities[ck] = true; states[s.state] = true; cityCount[ck] = (cityCount[ck] || 0) + 1;
                      }
                      ...
                      topList.innerHTML = "";
                      for (var j = 0; j < sorted.length; j++) { var li = document.createElement("li"); li.innerHTML = sorted[j][0] + " <span>(" + sorted[j][1] + ")</span>"; topList.appendChild(li); }
          }
          ...
          async function sendBatch(batch, batchNum, totalBatches) {
                      ...
                      var resp = await fetch(API_URL, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ images: batch }) });
          ...
          async function handleUpload(file) {
                      try {
                                    ...
                                    saveData(); updateShowFilter(); renderPins(); updateStats();
                      ...
                      } catch (err) { console.error("Upload error:", err); showStatus("Error: " + err.message, "error"); }
          }
          document.getElementById("pdfUpload").addEventListener("change", function (e) { var file = e.target.files[0]; if (file) { handleUpload(file); e.target.value = ""; } });
          document.getElementById("showFilter").addEventListener("change", function (e) { currentFilter = e.target.value; renderPins(); updateStats(); });
          window.addEventListener("resize", function () {
                      if (!svg || !mapReady) return;
                      var c = document.getElementById("map-container"); mapWidth = c.clientWidth; mapHeight = c.clientHeight || window.innerHeight - 70;
                      svg.attr("width", mapWidth).attr("height", mapHeight); projection.translate([mapWidth / 2, mapHeight / 2]).scale(mapWidth * 1.1);
                      path = d3.geoPath().projection(projection); g.selectAll(".state").attr("d", path); renderPins();
          });
