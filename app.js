(function () {
  ...
  async function loadCityCoords() {
    const resp = await fetch("https://gist.githubusercontent.com/Miserlou/c5cd8364bf9db202d03614d469613894/raw/2bf258763cdddd704f8ffd3ea9a3e81d25e2c6f6/cities.json");
  ...
  function renderPins() {
    ...
    for (const [key, data] of Object.entries(cityMap)) {
      ...
      pinG.on("mouseover", function (event) {
        ...
        tooltip.innerHTML = "<h4>" + data.city + ", " + data.state + "</h4><p>" + data.count + " shipment" + (data.count > 1 ? "s" : "") + "</p><p>Shows: " + showList + "</p>";
  ...
  function updateStats() {
    ...
    const sorted = Object.entries(cityCount).sort(function (a, b) { return b[1] - a[1]; }).slice(0, 10);
  ...
  async function handleUpload(file) {
    ...
    if (!showName) {
      showStatus("Please enter a show name before uploading.", "error");
      return;
    }
    ...
    try {
      ...
      const resp = await fetch(API_URL, {
        ...
        body: JSON.stringify({ pdfBase64: base64, showName: showName }),
      });
      ...
      showStatus("Success! Parsed " + result.newLocations.length + " locations. Total: " + result.totalShipments + " shipments.", "success");
