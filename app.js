(function () {
   ...
   async function handleUpload(file) {
             showStatus("Parsing shipping labels with AI... this may take up to 25 seconds for large PDFs.", "loading");
             try {
                         ...
                         console.log("Response status:", resp.status, "body length:", respText.length);
                         if (!respText || respText.trim().length === 0) {
                                       showStatus("Error: Server returned empty response. The PDF may be too large or the request timed out. Try a smaller PDF.", "error");
                                       return;
                         }
                         var result;
                         try {
                                       ...
                                       console.error("Failed to parse response:", respText.substring(0, 200));
                                       showStatus("Error: Server returned invalid response (status " + resp.status + "). Try again or use a smaller PDF.", "error");
                                       return;
                         }
                         if (!resp.ok) {
                                       showStatus("Error: " + (result.error || "Unknown error (status " + resp.status + ")"), "error");
                                       return;
                         }
